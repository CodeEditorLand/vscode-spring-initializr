...
export async function updatePom(
	...
	if (dependenciesNode !== undefined) {
		...
		await updateWorkspaceEdit(
			...
			uri,
			projectNode,
			new DependencyNodes(deps, { initParent: true }),
		);
	}
		...
		if (depMgmtNode !== undefined) {
			...
			if (depsNodes !== undefined) {
				...
				await updateWorkspaceEdit(
					...
					uri,
					depMgmtNode,
					new BOMNodes(boms, { parents: ["dependencies"] }),
				);
			}
		} else {
			await updateWorkspaceEdit(
				...
				projectNode,
				new BOMNodes(boms, {
					parents: ["dependencies", "dependencyManagement"],
				}),
			);
		}
	}
...
async function updateWorkspaceEdit(
	...
	const insPosLineStart: vscode.Position = new vscode.Position(
		insertPos.line,
		0,
	);
...
abstract class PomNode {
	protected static wrapWithParentNode(
		lines: string[],
		indent: string,
		parent: string,
	) {
...
class DependencyNodes extends PomNode {
	constructor(
		...
		private options?: { initParent?: boolean },
	) {
		super();
	}

	public getTextLines(indent: string): string[] {
		const listOfLines: string[] = [].concat(
...
class BOMNodes extends PomNode {
	constructor(
		private boms: IBom[],
		private options?: { parents?: string[] },
	) {
		super();
	}
