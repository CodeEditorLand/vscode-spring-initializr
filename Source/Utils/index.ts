...
export async function writeFileToExtensionRoot(
...
export async function readFileFromExtensionRoot(
	relateivePath: string,
): Promise<string> {
	const extensionRootPath: string =
		vscode.extensions.getExtension(getExtensionId()).extensionPath;
...
export function groupIdValidation(value: string): string | undefined {
	return /^[a-z_][a-z0-9_]*(\.[a-z0-9_]+)*$/.test(value)
		? undefined
		: "Invalid Group Id";
}
