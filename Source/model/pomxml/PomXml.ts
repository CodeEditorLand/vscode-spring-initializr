...
function getNode(
	...
	if (allowEmpty) {
		return isNullNode(parentNode[nodeName])
			? fallbackValue
			: parentNode[nodeName][0];
	} else {
		return isNullOrEmptyNode(parentNode[nodeName])
			? fallbackValue
			: parentNode[nodeName][0];
	}
}
...
export function addDependencyNode(
	...
	if (isNullOrEmptyNode(dependenciesNode.dependency)) {
		...
		if (
			!dependenciesNode.dependency.find((elem) => _.isEqual(elem, node))
		) {
...
export function removeDependencyNode(
	projectNode: XmlNode,
	groupId: string,
	artifactId: string,
): void {
	...
	if (!isNullOrEmptyNode(dependenciesNode.dependency)) {
		dependenciesNode.dependency = dependenciesNode.dependency.filter(
			(elem) =>
				!(
					groupId === elem.groupId[0] &&
					artifactId === elem.artifactId[0]
				),
		);
	}
}
...
export function addBomNode(projectNode: XmlNode, node: IBomNode): void {
	const dependencyManagementNode: XmlNode = ensureNode(
		projectNode,
...
export function addRepositoryNode(
	...
	if (isNullOrEmptyNode(repositoriesNode.repository)) {
		...
		// insert if not exists
		if (
			!repositoriesNode.repository.find((elem) => _.isEqual(elem, node))
		) {
