...
export class GenerateProjectHandler extends BaseHandler {
	...
	constructor(projectType: ProjectType, defaults?: IDefaultProjectData) {
		...
		this.metadata = {
			pickSteps: [],
			defaults: defaults || {},
			parentFolder: vscode.workspace
				.getConfiguration("spring.initializr")
				.get<ParentFolder>("parentFolder"),
		};
	}
	...
	public async runSteps(operationId?: string): Promise<void> {
		...
		this.outputUri = await instrumentOperationStep(
			operationId,
			"TargetFolder",
			specifyTargetFolder,
		)(this.metadata);
		...
		await instrumentOperationStep(
			...
			"DownloadUnzip",
			downloadAndUnzip,
		)(this.downloadUrl, this.outputUri);
		...
		if (
			...
			(vscode.workspace.workspaceFolders.some(
				(folder) => folder.uri.fsPath === this.outputUri.fsPath,
			) ||
				vscode.workspace.rootPath === this.outputUri.fsPath)
		) {
			return;
		}
		...
		if (choice === OPEN_IN_NEW_WORKSPACE) {
			...
			if (
				!vscode.workspace.workspaceFolders.find(
					(workspaceFolder) =>
						workspaceFolder.uri &&
						this.outputUri.fsPath.startsWith(
...
async function specifyTargetFolder(
	...
	const useArtifactId: boolean =
		metadata.parentFolder === ParentFolder.ARTIFACT_ID;

	const MESSAGE: string = useArtifactId
		? MESSAGE_EXISTING_FOLDER
		: MESSAGE_FOLDER_NOT_EMPTY;

	let outputUri: vscode.Uri = metadata.defaults.targetFolder
		? vscode.Uri.file(metadata.defaults.targetFolder)
		: await openDialogForFolder({ openLabel: LABEL_CHOOSE_FOLDER });

	if (outputUri && useArtifactId) {
		outputUri = vscode.Uri.file(
			`${outputUri.fsPath}/${metadata.artifactId}`,
		);
	}
	...
	while (
		(!useArtifactId &&
			outputUri &&
			(await vscode.workspace.fs.readDirectory(outputUri)).length > 0) ||
		(useArtifactId && outputUri && (await pathExists(outputUri)))
	) {
		const overrideChoice: string = await vscode.window.showWarningMessage(
			MESSAGE,
			OPTION_CONTINUE,
