...
export class SpecifyPackageNameStep implements IStep {
	...
	private static specifyPackageNameStep: SpecifyPackageNameStep =
		new SpecifyPackageNameStep();
	...
	private async specifyPackageName(
		projectMetadata: IProjectMetadata,
	): Promise<boolean> {
		const recommendedPackageName =
			`${projectMetadata.groupId}.${projectMetadata.artifactId}`.replace(
				"-",
				"_",
			);
