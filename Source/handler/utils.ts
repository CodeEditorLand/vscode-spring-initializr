...
export async function createPickBox<T extends Identifiable>(
	pickMetadata: IPickMetadata<T>,
): Promise<boolean> {
		...
		disposables.push(
			pickBox.onDidAccept(() => {
				...
				if (pickMetadata.pickStep instanceof SpecifyLanguageStep) {
					...
					pickMetadata.pickStep instanceof SpecifyJavaVersionStep
				) {
					pickMetadata.metadata.javaVersion =
						pickBox.selectedItems[0].value?.id;
				} else if (
					pickMetadata.pickStep instanceof SpecifyPackagingStep
				) {
					pickMetadata.metadata.packaging =
						pickBox.selectedItems[0].label?.toLowerCase();
				} else if (
					pickMetadata.pickStep instanceof SpecifyBootVersionStep
				) {
			...
			pickBox.onDidHide(() => {
				if (pickMetadata.pickStep instanceof SpecifyLanguageStep) {
					return reject(
						new OperationCanceledError("Language not specified."),
					);
				} else if (
					pickMetadata.pickStep instanceof SpecifyJavaVersionStep
				) {
					...
					return reject(
						new OperationCanceledError("Packaging not specified."),
					);
				} else if (
					pickMetadata.pickStep instanceof SpecifyBootVersionStep
				) {
	...
	const result: boolean = await new Promise<boolean>((resolve, reject) => {
		...
		disposables.push(
			...
			inputBox.onDidHide(() => {
				if (inputMetaData.pickStep instanceof SpecifyGroupIdStep) {
					return reject(
						new OperationCanceledError("GroupId not specified."),
					);
				} else if (
					inputMetaData.pickStep instanceof SpecifyArtifactIdStep
				) {
					return reject(
						new OperationCanceledError("ArtifactId not specified."),
					);
				} else if (
					inputMetaData.pickStep instanceof SpecifyPackageNameStep
				) {
