...
export class SpecifyPackagingStep implements IStep {
	...
	private static specifyPackagingStep: SpecifyPackagingStep =
		new SpecifyPackagingStep();
	...
	private async specifyPackaging(
		projectMetadata: IProjectMetadata,
	): Promise<boolean> {
		const packaging: string =
			projectMetadata.defaults.packaging ||
			workspace
