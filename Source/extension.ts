...
async function initializeExtension(
	...
	context.subscriptions.push(
		instrumentAndRegisterCommand(
			...
			async (operationId, defaults) =>
				await new GenerateProjectHandler("maven-project", defaults).run(
					operationId,
				),
			true,
		),
		instrumentAndRegisterCommand(
			...
			async (operationId, defaults) =>
				await new GenerateProjectHandler(
					"gradle-project",
					defaults,
				).run(operationId),
			true,
	...
	context.subscriptions.push(
		instrumentAndRegisterCommand(
			...
			async () => {
				const projectType: { value: string; label: string } =
			...
			async (_oid: string, entry?: vscode.Uri) => {
				...
				if (targetFile) {
					...
					vscode.window.showInformationMessage(
						"No pom.xml found in the workspace.",
					);
				}
			},
			true,
...
function instrumentAndRegisterCommand(
	name: string,
	cb: (...args: any[]) => any,
	withOperationIdAhead?: boolean,
): vscode.Disposable {
	const instrumented: (...args: any[]) => any = instrumentOperation(
		name,
		async (_operationId, ...args) => {
			withOperationIdAhead
				? await cb(_operationId, ...args)
				: await cb(...args);
		},
	);
